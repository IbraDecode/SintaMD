...
if (!conn.authState.creds.registered) {
  ...
  setTimeout(async () => {
    let code = await conn.requestPairingCode(global.pairing, "SINTANIH")
...
async function connectionUpdate(update) {
  ...
  if (connection == 'connecting') {
    ...
    // Auto join channel
    try {
      await conn.newsletterFollow('120363406301359528@newsletter')
  ...
  if (connection == 'close') {
    console.log(chalk.red('[ X ] Koneksi terputus & mencoba menyambung ulang...'));
  }
...
async function filesInit() {
  for (let filename of readdirSync(pluginFolder).filter(pluginFilter)) {
    try {
      ...
      conn.logger.error(`[ X ] Failed to load plugins ${filename}: ${e}`)
...
_quickTest()
  .then(() => conn.logger.info('[ X ] Quick Test Done , nama file session ~> creds.json'))
  .catch(console.error);
